...
# Custom Feature Definition File
    ...
    *** MolGraph Class ***
    ...
    def get_pharmacophore_features(self, mol: Chem.rdchem.Mol, feature_factory: Chem.rdMolChemicalFeatures.MolChemicalFeatureFactory = factory) -> list:
        ...
        for atom in mol.GetAtoms():
            ...
            atom_prop['Toxic'] = atom_prop.pop('Toxicophore') 
    ...
    def get_nodes(self):
        ...
        node_colours = [self.atom_colours[str(atom_labels[n])] for n in nodes] #milan
        ...
        for n in N:
            ...
            for neighbor in neighbors:
                if graph.has_edge(n,neighbor):
                    ...
                    if b is None:
                        pass # changed for artificial bond
                    else:
                        bond_order.append(b)
...
class ConflictGraph:
    ...
    def get_potential_matches(self):
        ...
        Vc_K = [(u, v, [key for key in self.critical_labels
                      ...
                      if key in G1.nodes[u] and key in G2.nodes[v] and G1.nodes[u][key] == G2.nodes[v][key]])# and str(G1.nodes[u][key])!= str(False)])
                ...
                for u in G1.nodes() for v in G2.nodes() if (any(key for key in self.critical_labels
                                                               if key in G1.nodes[u] and key in G2.nodes[v] and G1.nodes[u][key] == G2.nodes[v][key]))# and G1.nodes[u][key]!= False))
                                                               and ( G1.nodes[u].get('ring') == G2.nodes[v].get('ring'))]
